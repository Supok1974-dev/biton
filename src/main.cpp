...
#if defined(NDEBUG)
# error "Biton cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9be5eec82af34cfcc7ddca83f8e512cdc43f361affec3203a4d554a3db77139f"); 
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Biton: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Biton Signed Message:\n";
...
{
    // Biton: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Biton: 1000k blocks in ~2 year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Biton: 5 minutes
static const int64 nTargetSpacing = 1 * 60; // Biton: 60 Seconds.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Biton: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Biton: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9be5eec82af34cfcc7ddca83f8e512cdc43f361affec3203a4d554a3db77139f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "02/Dec/2015 BITON.pw";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("88ed3ceb73d667eb256b461e5924cc2d596a7ac936c293bbb235d3e28f73e8ce9015ef91a62cfd83fc7d8ec9aad57e686f511caf4ad702fd41435dcda9e936ac12") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1449060000;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1849402;
        ...
        assert(block.hashMerkleRoot == uint256("0xa4ae200c100f3c1600b73df3a2d2bd104f913ab17b47294e492dc7a9093fd227"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // Biton: increase each by adding 2 to biton's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitonMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitonMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitonMiner(CWallet *pwallet)
{
    printf("BitonMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("biton-miner");
    ...
    try { loop {
        ...
        printf("Running BitonMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitonMiner terminated\n");
        throw;
    }
}

void GenerateBitons(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitonMiner, pwallet));
}
