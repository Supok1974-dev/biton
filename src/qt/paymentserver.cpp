...
const int BITON_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString BITON_IPC_PREFIX("biton:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(BITON_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(BITON_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(BITON_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click biton: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start biton: click-to-pay handler");
    else
...
{
    // clicking on biton: URLs creates FileOpen events on the Mac:
