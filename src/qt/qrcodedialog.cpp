...
{
    QString ret = QString("biton:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BITON unit input in lnReqAmount, we generate the URI with BITON as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitonUnits::format(BitonUnits::BITON, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
